...
var convertVariables = function convertVariables(str) {
  return lodash.isString(str) ? str.replace(/\{\{([^}]+)\}\}/g, '{$.ctx.$1}') : str;
};
...
var replaceVariables = function replaceVariables(source) {
  if (!lodash.isEmpty(source)) {
    ...
    if (lodash.isPlainObject(source)) {
      return lodash.mapValues(source, function (val) {
        return lodash.isString(val) ? convertVariables(val) : val;
      });
    }
  }
...
var createRequestHeaders = function createRequestHeaders(request) {
  var modeHeaders = convertModeToHeader(lodash.get(request, 'body.mode'));
  var headers = {};

  if (!lodash.isEmpty(request.header)) {
    request.header.forEach(function (header) {
      headers[header.key] = header.value;
    });
  }

  return replaceVariables(lodash.merge(headers, modeHeaders));
};
...
var createRequestBody = function createRequestBody(request) {
  var mode = lodash.get(request, 'body.mode');
  var body = lodash.get(request, ['body', mode]);
  var result = void 0;
...
var generateId = function generateId() {
  return index$1.generate().substring(0, 3).toLowerCase();
};
...
var createAuth = function createAuth() {
  ...
  switch (type) {
    case 'basic':
      result = lodash.pick(authObj, ['username', 'password']);
      break;
    case 'oauth1':
      result = _extends({}, lodash.pick(authObj, ['consumerKey', 'consumerSecret', 'token', 'tokenSecret', 'signatureMethod']), {
...
var createLogic = function createLogic(item, type) {
  var event = lodash.find(item.event, { listen: type });

  if (event) {
    var exec = event.script.exec;


    if (lodash.isArray(exec)) {
      return {
        script: exec.join('\n')
      };
    }
...
var convert = function convert(collection) {
  ...
  var scenarios = lodash.get(collection, 'scenarios', []);

  if (!lodash.isEmpty(collection.item)) {
  ...
  return {
    name: lodash.get(collection, 'name') || lodash.get(collection, 'info.name') || '',
    scenarios: scenarios
  };
};
