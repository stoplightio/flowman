...
var convertVariables = function convertVariables(str) {
  return lodash.isString(str) ? str.replace(/\{\{([^}]+)\}\}/g, '<<!$1>>') : str;
};
  ...
  switch (type) {
    case 'basic':
      result = defineProperty({
        type: type
      }, type, lodash.pick(authObj, ['username', 'password']));
      break;
    case 'oauth1':
      result = defineProperty({
        type: type
      }, type, _extends({}, lodash.pick(authObj, ['consumerKey', 'consumerSecret', 'tokenSecret', 'signatureMethod']), {
...
var createFlow = function createFlow(item) {
  var flow = {
    name: item.name,
    flowVersion: '1.0',
    resourceId: lodash.kebabCase(item.name),
    steps: []
  };
...
var convert = function convert(collection) {
  ...
  return {
    name: lodash.get(collection, 'info.name') || '',
